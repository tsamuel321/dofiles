...
replace pl = 3.2 if inlist(countrycode,"SYR","TUV","VEN","XKX","YEM")  //These are the countries with survey data that do not have 2017 PPPs.
...
forvalues row=1/`r(N)' {
	...
	*loc pl =  pl2017[`row']
	...
	capture povcalnet, country(`ccc') year(all) povline(3.2) coverage(national) fillgaps clear   

	save `"${dir}\data/poverty_`ccc'_2011.dta"', replace
}

*Argentina
povcalnet, country(ARG) year(all) coverage(urban) povline(3.2) fillgaps clear     //1. Argentina has urban coverage, so I get its data separately.
...
*Scatter plot (slide #4)
	...
	replace pos = 12 if countrycode=="NER" 
	replace pos = 12 if countrycode=="IDN"
	replace pos = 6 if countrycode=="IND"
	replace pos = 12 if countrycode=="HTI"
	replace pos = 9 if countrycode=="TJK"
	replace pos = 12 if countrycode=="FSM"
	replace pos = 12 if countrycode=="GEO"
	replace pos = 12 if countrycode=="EGY"
	replace pos = 12 if countrycode=="DJI"
	replace pos = 5 if countrycode=="PAK"
	replace pos = 9 if countrycode=="GMB"
	...
	replace countrycode="" if  diff< 8.5  | countrycode=="LSO"
...
preserve
	...
	export excel using `"${dir}\results\Country_poverty_numbers_4_0.xlsx"', sheet("poor") firstrow(var) replace       
restore
...
preserve
	...
	export excel using `"${dir}\results\Country_poverty_numbers_4_0.xlsx"', sheet("povrate") firstrow(var)
restore
...
preserve
	...
	export excel using `"${dir}\results\Global_poverty_4_0.xlsx"', sheet("regions") firstrow(varl) replace
restore
...
preserve
	...
	export excel using `"${dir}\results\Global_poverty_4_0.xlsx"', sheet("countries", modify) firstrow(var) 
	...
	export excel using `"${dir}\results\Global_poverty_4_0.xlsx"', sheet("world", modify) firstrow(varl)
restore
