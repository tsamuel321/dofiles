...
replace pl = 5.5 if inlist(countrycode,"SYR","TUV","VEN","XKX","YEM")  //These are the countries with survey data that do not have 2017 PPPs.
...
forvalues row=1/`r(N)' {
	...
	*loc pl =  pl2017[`row']
	...
	capture povcalnet, country(`ccc') year(all) povline(5.5) coverage(national) fillgaps clear   

	save `"${dir}\data/poverty_`ccc'_2011.dta"', replace
}

*Argentina
povcalnet, country(ARG) year(all) coverage(urban) povline(5.5) fillgaps clear     //1. Argentina has urban coverage, so I get its data separately.
...
*Scatter plot (slide #4)
	...
	replace pos = 3 if countrycode=="STP"
	replace pos = 11 if countrycode=="MDG"
	replace pos = 4 if countrycode=="TON"
	replace pos = 4 if countrycode=="WSM"
	replace pos = 2 if countrycode=="BOL"
	replace pos = 8 if countrycode=="BLZ"
	replace pos = 12 if countrycode=="TJK"
	replace pos = 12 if countrycode=="LKA"
	replace pos = 2 if countrycode=="TKM"
...
merge 1:m countrycode year coverage using `"${dir}\data/pop.dta"', gen(m1)

preserve
*Get a file for countries not available in povcalnet

	keep if m1==2                         //these are countries not available in povcalnet
...
preserve
	...
	export excel using `"${dir}\results\Country_poverty_numbers_6_3.xlsx"', sheet("poor") firstrow(var) replace       
restore
...
preserve
	...
	export excel using `"${dir}\results\Country_poverty_numbers_6_3.xlsx"', sheet("povrate") firstrow(var)
restore
...
preserve
	...
	export excel using `"${dir}\results\Global_poverty_6_3.xlsx"', sheet("regions") firstrow(varl) replace
restore
...
preserve
	collapse headcount_2011 headcount_2017 pop if year==2015, by(countryname year)
	...
	export excel using `"${dir}\results\Global_poverty_6_3.xlsx"', sheet("countries", modify) firstrow(var) 
	...
	export excel using `"${dir}\results\Global_poverty_6_3.xlsx"', sheet("world", modify) firstrow(varl)
restore
...
twoway line headcount_regavg_2011 headcount_regavg_2017 year if  year<=2015,  ///
	ytitle("Poverty rate, %") xtitle("") graphregion(color(white))  ylabel(0(20)100) xlabel(1995(10)2015)  ///
