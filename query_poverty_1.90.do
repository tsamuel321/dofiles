...
gen pl2011_new = 1.9*ppp2011_revised/ppp2011

drop if inlist(countrycode,"CHN","IND","IDN") & coveragetype!="National"
...
*keep if inlist(countrycode,"ARE","BGD","BLZ","KEN") | inlist(countrycode,"KOR","LVA","MDV","MLI")

*save `"${dir}\data/survey_query_ppp.dta"', replace


keep if inlist(countrycode,"DOM")
...
forvalues row=1/`r(N)' {
	...
	use `"${dir}\data/survey_query_ppp_missing.dta"', clear
	...
	capture povcalnet, country(`ccc') year(all) coverage(`coverage') povline(`pl') clear fillgaps
...
label var headcount_2011_new "Poverty rate (with new 2011 PPPs)"
...
*keep if inlist(countrycode,"BTN","DJI","SLB")
...
forvalues row=1/`r(N)' {
	...
	*loc pl = pl2011_new[`row']
	...
	if "`ccc'"=="ARG" 		local coverage="urban"
	 ...
	 capture povcalnet, country(`ccc') year(all) povline(1.9) coverage(`coverage') clear fillgaps

	save `"${dir}\data/poverty_`ccc'_2011.dta"', replace
}

*Argentina
*povcalnet, country(ARG) year(all) coverage(urban) povline(1.9) fillgaps clear     //1. Argentina has urban coverage, so I get its data separately.
...
merge 1:1 countrycode year coveragetype using `"${dir}\data/poverty_2011_new_all.dta"', keep(3) nogen
...
*Scatter plot (slide #4)
	...
	*replace countryname="" if countrycode!="TLS"

	replace countryname="" if  (diff> -.012 & diff < 0.018) 
	
	gen pos = 11

	replace pos = 9 if countryname=="Nigeria" 
	replace pos = 5 if diff < -.012
	replace pos = 3 if countryname=="Indonesia"
	replace pos = 4 if countryname=="Romania"
	replace pos = 9 if countryname=="Angola"
	replace pos = 12 if countryname=="Liberia"
...
/*
preserve
povcalnet, country(all) year(2018) fillgaps clear
keep countrycode
duplicates drop
tempfile pcn
save `pcn'
restore 
merge m:1 countrycode using `pcn', nogen
*/
...
keep countrycode countryname headcount_2011 headcount_2011_new population coverage year
...
preserve
	...
	export excel using `"${dir}\results\Country_poverty_numbers_1_90.xlsx"', sheet("poor") firstrow(var) replace       
restore
...
preserve
	...
	export excel using `"${dir}\results\Country_poverty_numbers_1_90.xlsx"', sheet("povrate") firstrow(var)
restore
...
preserve
	...
	export excel using `"${dir}\results\Global_poverty_1_90.xlsx"', sheet("regions") firstrow(varl) replace
restore
...
*Global and regional aggregation (w/ old & new 2011 PPPs) in 2015
preserve
	...
	export excel using `"${dir}\results\Global_poverty_1_90.xlsx"', sheet("countries", modify) firstrow(var) 
	...
	export excel using `"${dir}\results\Global_poverty_1_90.xlsx"', sheet("world", modify) firstrow(varl)
restore
...
collapse headcount_regavg_2011 headcount_regavg_2011_new,by(region year) 
...
collapse headcount_2011 headcount_2011_new pop, by(countryname year)
bysort year: egen headcount_gloavg_2011 = wtmean(headcount_2011), weight(pop)
bysort year: egen headcount_gloavg_2011_new = wtmean(headcount_2011_new), weight(pop)
...
collapse headcount_gloavg_2011 headcount_gloavg_2011_new,by(year) 
...
twoway line headcount_regavg_2011 headcount_regavg_2011_new year if  year<=2015,  ///
	ytitle("Poverty rate, %") xtitle("") graphregion(color(white))  ylabel(0(10)60) xlabel(1995(10)2015)  ///
	legend(lab(1 "w/ original 2011 PPPs") lab(2 "w/ revised 2011 PPPs")) by(region, row(2) style(compact) note("") iscale(*.9) ) 

save `"${dir}\data/regional_trends_r2011.dta"', replace
