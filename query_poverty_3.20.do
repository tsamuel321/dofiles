...
gen pl2011_new = 3.2*ppp2011_revised/ppp2011
*gen pl2011_new_ =  3.4*ppp2011_revised/ppp2011
...
*keep if inlist(countrycode,"ARE")
...
NOR NPL PAK PAN PER PHL POL PRT PRY PSE ROU RUS RWA SDN SEN SLE SLV SRB STP SUR SVK SVN SWE SWZ SYC TCD TGO THA TJK TTO TUN TUR TZA UGA URY USA VEN VNM YEM ZAF ZMB ZWE FSM GEO GUY IRN KIR LBN PNG SLB SSD SYR TKM TLS TON TUV UKR UZB VUT WSM XKX TWN ARE
";
...
/*
///////////Get poverty headcounts with 3.40 line (revised 2011 PPP)/////////////////////

*Load survey query file with PPP data
use `"${dir}\data/survey_query_ppp.dta"', clear

*keep if inlist(countrycode,"ARG")



*save `"${dir}\data/survey_query_ppp_missing_new_.dta"', replace
...
forvalues row=1/`r(N)' {
	...
	*use `"${dir}\data/survey_query_ppp_missing_new.dta"', clear
...
#delimit cr;  
 ...
 foreach x of local ccc {
	capture append using `"${dir}\data/poverty_`x'_2011_new_.dta"', force
	
	}
	
rename headcount headcount_2011_new_
label var headcount_2011_new_ "Poverty rate at 3.4 (with new 2011 PPPs)"
...
*keep if inlist(countrycode,"IRN")

*save `"${dir}\data/survey_query_ppp_missing_old.dta"', replace
...
forvalues row=1/`r(N)' {
	...
	*loc pl = pl2011_new[`row']
	...
	capture povcalnet, country(`ccc') year(all) coverage(`coverage') povline(3.2) fillgaps clear  
...
preserve
	...
	export excel using `"${dir}\results\Country_poverty_numbers_3_20.xlsx"', sheet("poor", modify) firstrow(var)      
restore
...
preserve
	...
	export excel using `"${dir}\results\Country_poverty_numbers_3_20.xlsx"', sheet("povrate", modify) firstrow(var)
restore
...
*Regional aggregation  (w/ new 2011 PPPs at 3.4 line)
*bysort region year: egen headcount_regavg_2011_new_ = wtmean(headcount_2011_new_), weight(pop)
*replace headcount_2011_new_ = headcount_regavg_2011_new_ if missing(headcount_2011_new_)     //Assign to countries without povcalnet poverty numbers the poverty rate of their region
...
preserve
	...
	export excel using `"${dir}\results\Global_poverty_3_20.xlsx"', sheet("regions", modify) firstrow(varl) 
restore
...
preserve
	...
	export excel using `"${dir}\results\Global_poverty_3_20.xlsx"', sheet("countries", modify) firstrow(var) 
	...
	export excel using `"${dir}\results\Global_poverty_3_20.xlsx"', sheet("world", modify) firstrow(varl)
restore
...
twoway line headcount_regavg_2011 headcount_regavg_2011_new  year if  year<=2015,  ///
	...
	legend(row(1) lab(1 "w/ original 2011 PPPs") lab(2 "w/ revised 2011 PPPs")) by(region, row(2) style(compact) note("") iscale(*.9) ) 
